import {
    DecoratorMetadata, DEFAULT_ERROR_MESSAGES,
    Errors,
    ModelErrors,
    ValidationKeys,
...
import {constructFromModel, constructFromObject, ModelBuilderFunction} from "./construction";
...
export function validate<T extends Model>(obj: T, ...propsToIgnore: string[]) : ModelErrorDefinition | undefined {
    ...
    for (let prop in obj)
        if (obj.hasOwnProperty(prop) && propsToIgnore.indexOf(prop) === -1)
            decoratedProperties.push(getPropertyDecorators(ValidationKeys.REFLECT, obj, prop) as ValidationPropertyDecoratorDefinition);
    ...
    const result = decoratedProperties.reduce((accum: undefined | ModelErrors, decoratedProperty: ValidationPropertyDecoratorDefinition) => {
        ...
        let errs: { [indexer: string]: Errors } | undefined = decorators.reduce((acc: undefined | { [indexer: string]: Errors }, decorator: { key: string, props: {} }) => {
            ...
            let err: Errors = validator.hasErrors(obj[prop.toString()], ...(decorator.key === ModelKeys.TYPE ? [decorator.props] : Object.values(decorator.props)));
                        ...
                        switch (c){
                            ...
                            case "Set":
                                if (allDecorators.length){
                                    ...
                                    if (listDec) {
                                        const e = c === "Array"
                                            ? (obj as Record<string, any>)[prop].find((c: Validatable) => c.hasErrors())
                                            : (obj as Record<string, any>)[prop].values().find((c: Validatable) => c.hasErrors());
                                        if(e)
                                            err = stringFormat(DEFAULT_ERROR_MESSAGES.LIST_INSIDE,  e.toString())
                                    }
                                }
                                break;
                            default:
