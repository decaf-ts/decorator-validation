...
export class EqualsValidator extends Validator<ComparisonValidatorOptions> {
  ...
  public hasErrors(
    ...
    let propertyToMatchValue: any;
    try {
      propertyToMatchValue = getValueByPath(instance, options.propertyToMatch);
    } catch (e: any) {
      return this.getMessage(e.message || this.message);
    }

    return isEqual(value, propertyToMatchValue)
      ? undefined
      : this.getMessage(
          options.message || this.message,
          options.propertyToMatch
        );
  }
}
